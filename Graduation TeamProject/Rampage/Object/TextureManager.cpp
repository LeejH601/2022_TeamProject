...
void CTextureManager::CreateResourceView(ID3D12Device* pd3dDevice, UINT nDescriptorHeapIndex)
{
	...
	m_iBillBoardTexture = std::make_pair(1, m_iBillBoardTextureN - 1);
	for (int i = 1; i < m_iBillBoardTextureN - 1; i++)
	{
		ID3D12Resource* pShaderResource = m_pTextures->GetResource(i);
		D3D12_SHADER_RESOURCE_VIEW_DESC d3dShaderResourceViewDesc = m_pTextures->GetShaderResourceViewDesc(i);
		pd3dDevice->CreateShaderResourceView(pShaderResource, &d3dShaderResourceViewDesc, m_d3dSrvCPUDescriptorNextHandle);
	...
	m_iParticleTexture = std::make_pair(m_iBillBoardTextureN, m_iBillBoardTextureN + m_iParticleTextureN - 1);
	for (int i = m_iBillBoardTextureN; i < m_iBillBoardTextureN + m_iParticleTextureN - 1; i++)
	{
		ID3D12Resource* pShaderResource = m_pTextures->GetResource(i);
		D3D12_SHADER_RESOURCE_VIEW_DESC d3dShaderResourceViewDesc = m_pTextures->GetShaderResourceViewDesc(i);
		pd3dDevice->CreateShaderResourceView(pShaderResource, &d3dShaderResourceViewDesc, m_d3dSrvCPUDescriptorNextHandle);
...
void CTextureManager::LoadSphereBuffer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	m_pTextures->CreateBuffer(pd3dDevice, pd3dCommandList, pxmf4RandomValues, 1024, sizeof(XMFLOAT4), DXGI_FORMAT_R32G32B32A32_FLOAT, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_GENERIC_READ, 0/*, RESOURCE_BUFFER*/);
}
...
//}
//
int CTextureManager::LoadTextureIndex(LPCTSTR pszFileName)
{
	for (int i = 0; i < m_pTextures->GetTextures(); i++)
	{
		if (!wcscmp(m_pTextures->GetTextureName(i), pszFileName))
		{
			return i;
...
std::shared_ptr<CTexture> CTextureManager::GetTexture()
{
	return m_pTextures;
}
std::pair<int, int> CTextureManager::GetTextureListIndex(TextureType eTextureType)
{
	switch (eTextureType)
	{
	case TextureType::SphereTexture:
		return m_iSphereTexture;
	case TextureType::BillBoardTexture:
		return m_iBillBoardTexture;
	case TextureType::ParticleTexture:
		return m_iParticleTexture;
	default:
