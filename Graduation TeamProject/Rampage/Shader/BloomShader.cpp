...
D3D12_SHADER_BYTECODE CBloomShader::CreateComputeShader(ID3DBlob** ppd3dShaderBlob, int nPipelineState)
{
	if(nPipelineState == 0)
		...
		return(CShader::ReadCompiledShaderFile(L"BloomUpSampleComputeShader.cso", ppd3dShaderBlob));
...
{
	if (m_ppd3dPipelineStates.size() != 4)
	{
		m_nPipelineStates = 4;
	...
	d3dPipelineStateDesc2.CS = CreateComputeShader(&pd3dComputeShaderBlob2, 3);
	hResult = pd3dDevice->CreateComputePipelineState(&d3dPipelineStateDesc2, __uuidof(ID3D12PipelineState), (void**)m_ppd3dPipelineStates[3].GetAddressOf());
...
void CBloomShader::CreateBloomUAVResource(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, UINT xResoution, UINT yResoultion, int nDownSampling)
{
	...
	for (int i = 0; i < nDownSample + 1; ++i) {
		m_pFillterTextures->CreateTexture(pd3dDevice, SampleTextureResoultions[i].x, SampleTextureResoultions[i].y, DXGI_FORMAT_R16G16B16A16_FLOAT, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, NULL, RESOURCE_TEXTURE2D, i, 1);
		m_pBluredTextures->CreateTexture(pd3dDevice, SampleTextureResoultions[i].x, SampleTextureResoultions[i].y, DXGI_FORMAT_R16G16B16A16_FLOAT, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, NULL, RESOURCE_TEXTURE2D, i, 1);
	}
