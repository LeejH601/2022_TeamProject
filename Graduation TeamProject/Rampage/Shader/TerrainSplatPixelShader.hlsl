...
PS_MULTIPLE_RENDER_TARGETS_OUTPUT PSTerrainSplat(VS_TERRAIN_OUTPUT input)
{
	...
	float3 vToCamera = normalize(vCameraPosition - input.position);

	float fHeight = FNormalColor.a;
	fHeight = fHeight * 0.08f + 0.04f;
	...
	float4 cIllumination = Lighting(input.position, normalW);
